...
import { SearchMapsRequest, SearchMapsRequestStatus } from '../../Autogenerated/Backend';
...
export const SearchMapsRequestListComponent: FC = () => {
    ...
    useEffect(() => {
        async function loadData() {
            let requests;
            if (isMod) {
                requests = await SearchMapsRequestAPI.GetAllRequests(beginDate, endDate, username);
            } else {
                requests = await SearchMapsRequestAPI.GetRequests();
                setRequests(requests);
            }
            const filteredRequests = requests?.filter(x => x.status != SearchMapsRequestStatus.Deleted);
